...
> 注意：此处 `-c` 是 `--category` 的缩写，**不是**其他命令里的 `--config`。`--name` 需配合 `--category` 使用才生效。
...
配置文件按以下顺序选择：`~/.aliyun/alibaba-cloud-idaas.json` 存在时优先使用，否则使用 `~/.cloud_idaas/idaas-cli.json`。

缓存目录随配置文件位置变化（加密存放 oidc_token / cloud_token / token_response）：
